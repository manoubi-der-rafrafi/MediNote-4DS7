...
class OrchestratorService:
    ...
    def _dispatch(
        ...
        if intent == "task_report":
            return self.task_report_service.handle(
                parsed_response,
                user_id=user_id,
                conversation_id=conversation_id,
            )
        ...
        if intent == "pdf_export":
            # Get the report content from task_report service - use 'rapports' since that's where your data is
            report_type = parsed_response.get("report_type", "rapports")
            
            # If report_type is 'full' or 'tasks' and no user_id, default to 'rapports'
            if report_type in ("full", "tasks", "conversations") and not user_id:
                report_type = "rapports"
            
            report_result = self.task_report_service.handle(
                {
                    "intent": "task_report",
                    "action": "generate_report",
                    "report_type": report_type,
                },
                user_id=user_id if report_type != "rapports" else None,
                conversation_id=conversation_id,
            )
            
            if report_result.get("status") == "success" and report_result.get("content"):
                pdf_result = self.pdf_export_service.export_report(
                    PdfExportRequest(
                ...
                return {
                    "status": "error",
                    "message": "No report content found to export",
                    "error": report_result.get("message", "No content available for PDF export")
                }
        # =================================================
    ...
    def _build_response_payload(
        ...
        if action == "query_database" and isinstance(dispatched_result, dict):
            ...
            data = dispatched_result
        # ========== ADDED FOR TASK REPORT AGENT ==========
        elif intent == "task_report" and isinstance(dispatched_result, dict):
            status = str(dispatched_result.get("status", "success")).strip() or "success"
            choices = list(dispatched_result.get("choices", []))
            missing_fields = list(dispatched_result.get("missing_fields", []))
            data = {
        ...
        # ========== ADDED FOR PDF EXPORT ==========
        elif intent == "pdf_export" and isinstance(dispatched_result, dict):
            status = str(dispatched_result.get("status", "success")).strip() or "success"
            data = dispatched_result
        # =================================================
    ...
    def _build_fallback_message(
        ...
        # ========== ADDED FOR TASK REPORT AGENT ==========
        if status == "success" and intent == "task_report" and isinstance(data, dict):
            ...
            return "Le rapport a ete genere avec succes."
        # ========== ADDED FOR PDF EXPORT ==========
        if status == "success" and intent == "pdf_export" and isinstance(data, dict):
            pdf_url = data.get("pdf_url")
            if pdf_url:
                if is_arabic:
                    return f"تم تصدير التقرير إلى PDF بنجاح. يمكنك تحميله من: {pdf_url}"
                if is_english:
                    return f"Report exported to PDF successfully. You can download it at: {pdf_url}"
                return f"Le rapport a ete exporte en PDF avec succes. Vous pouvez le telecharger ici: {pdf_url}"
            if is_arabic:
                return "تم تصدير التقرير إلى PDF بنجاح."
            if is_english:
                return "Report exported to PDF successfully."
            return "Le rapport a ete exporte en PDF avec succes."
        # =================================================
        ...
        if status == "missing_information":
            ...
            # ========== ADDED FOR TASK REPORT AGENT ==========
            if intent == "task_report":
                missing = response_payload.get("missing_fields", [])
                if "user_id" in missing:
                    if is_arabic:
                        return "معرف المستخدم مطلوب لإنشاء هذا التقرير."
                    if is_english:
                        return "User ID is required to generate this report."
                    return "L'ID utilisateur est requis pour generer ce rapport."
                if "conversation_id" in missing:
                    if is_arabic:
                        return "معرف المحادثة مطلوب لإنشاء تقرير الرسائل."
                    if is_english:
                        return "Conversation ID is required to generate the messages report."
                    return "L'ID de conversation est requis pour generer le rapport des messages."
            # =================================================
            if is_arabic:
