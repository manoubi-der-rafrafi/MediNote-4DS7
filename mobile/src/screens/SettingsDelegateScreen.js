// ─── screens/SettingsDelegateScreen.js — Delegate Profile & Settings ────────
...
export default function SettingsDelegateScreen({ navigation, onLogout }) {
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.section}>
          <Text style={s.sectionTitle}>Profil Délégué</Text>
          <View style={s.card}>
            ...
            <View style={s.card}>
              ...
              <TouchableOpacity style={[s.settingRow, { marginTop: spacing.md }]}>
                ...
                <Switch
                  ...
                  thumbColor={security.twoFactorEnabled ? colors.blue : colors.t2}
                />
              </TouchableOpacity>
            </View>
          )}
          ...
          <View style={s.card}>
            <TouchableOpacity style={s.settingRow}>
              <View>
                ...
                <Text style={s.settingSubtitle}>Recevoir les alertes par email</Text>
              </View>
              <Switch
                value={notifications.emailAlerts}
                onValueChange={(val) => updateNotificationSettings({ emailAlerts: val })}
                trackColor={{ false: colors.s3, true: colors.blue }}
                thumbColor={notifications.emailAlerts ? colors.blue : colors.t2}
              />
            </TouchableOpacity>
            <TouchableOpacity style={[s.settingRow, { marginTop: spacing.md }]}>
              <View>
                <Text style={s.settingTitle}>Notifications Push</Text>
                <Text style={s.settingSubtitle}>Notifications sur l'appareil</Text>
              </View>
              <Switch
                value={notifications.pushNotifications}
                onValueChange={(val) => updateNotificationSettings({ pushNotifications: val })}
                trackColor={{ false: colors.s3, true: colors.blue }}
                thumbColor={notifications.pushNotifications ? colors.blue : colors.t2}
              />
            </TouchableOpacity>
            <TouchableOpacity style={[s.settingRow, { marginTop: spacing.md }]}>
              <View>
                <Text style={s.settingTitle}>Rapport Hebdomadaire</Text>
                <Text style={s.settingSubtitle}>Email le lundi matin</Text>
              </View>
              <Switch
                value={notifications.weeklyReport}
                onValueChange={(val) => updateNotificationSettings({ weeklyReport: val })}
                trackColor={{ false: colors.s3, true: colors.blue }}
                thumbColor={notifications.weeklyReport ? colors.blue : colors.t2}
              />
            </TouchableOpacity>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  backTxt:        { color: colors.blue, fontSize: font.sm, fontWeight: '600' },
  pageTitle:      { color: colors.t1, fontSize: font.xl, fontWeight: '800' },
  section:        { marginBottom: spacing.lg },
  sectionTitle:   { color: colors.t2, fontSize: font.sm, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: spacing.sm },
  card:           { backgroundColor: colors.s1, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.bd, padding: spacing.lg, marginBottom: spacing.md },
  profileHeader:  { flexDirection: 'row', alignItems: 'flex-start', marginBottom: spacing.lg },
  avatarLarge:    { width: 64, height: 64, borderRadius: 32, backgroundColor: '#4F8EF7' + '22', borderWidth: 2, borderColor: colors.blue, alignItems: 'center', justifyContent: 'center' },
  avatarText:     { fontSize: font.xxl },
  changeAvatarText: { color: colors.blue, fontSize: font.xs, marginTop: 4, fontWeight: '600', textAlign: 'center' },
  ...
  statValue:      { color: colors.blue, fontSize: font.base, fontWeight: '700' },
